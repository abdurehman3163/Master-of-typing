...
//    }
    ...
    static var appLanguageCode: String? {
        get {
            return defaults.string(forKey: userDefaultsKey) // "selectedLanguageCode"
        }
        set {
            defaults.set(newValue, forKey: userDefaultsKey)
            // Trigger language change
            Localize.setCurrentLanguage(newValue ?? "en")
            NotificationCenter.default.post(name: .languageDidChange, object: nil)
        }
    }

    static var currentLanguage: Languages? {
        guard let code = appLanguageCode else { return nil }
