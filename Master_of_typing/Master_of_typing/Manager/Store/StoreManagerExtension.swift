...
extension StoreManager {
    var weekly: ProductInfo? {
        products.first(where: { $0.id == "com.rs.typing.master.weekly" })
    }

    var monthly: ProductInfo? {
        products.first(where: { $0.id == "com.rs.typing.master.monthly" })
    }

    var yearly: ProductInfo? {
        products.first(where: { $0.id == "com.rs.typing.master.yearly" })
    }

    var lifetime: ProductInfo? {
        products.first(where: { $0.id == "com.rs.typing.master.lifetime" })
    }
