...
class ProCVC: NSCollectionViewItem {
    ...
    func configure(weekly product: ProductInfo?) {
        ...
        weeklyLabel.stringValue = "Weekly"
        currentPriceLabel.stringValue = product.displayPrice
        basicPlaneLabel.stringValue = "Basic"
//        offPersentBox.fillColor = .leftVCBackground
        perWeekPriceLabel.stringValue = product.currencySymbol + String(format: "%.2f", product.price*2) + "/" + "Week"
        perWeekPriceLabel.textColor = .black
    ...
    func configure(monthly product: ProductInfo?) {
        guard let product else { return }
        weeklyLabel.stringValue = "Monthly"
        currentPriceLabel.stringValue = product.displayPrice
        basicPlaneLabel.stringValue = "Free Trial"
//        offPersentBox.fillColor = .proOrange
        perWeekPriceLabel.stringValue = product.currencySymbol + String(format: "%.2f", product.price/4) + "/" + "Week"
    ...
    func configure(yearly product: ProductInfo?) {
        guard let product else { return }
        weeklyLabel.stringValue = "Yearly"
        currentPriceLabel.stringValue = product.displayPrice
        perWeekPriceLabel.stringValue = product.currencySymbol + String(format: "%.2f", product.price / 52) + "/" + "Week"
        if let weeklyProduct = StoreManager.shared.weekly {
            ...
            basicPlaneLabel.stringValue = "Save" + " \(Int(discount))%"
        }
//        offPersentBox.fillColor = .proPurple
    ...
    func configure(lifetime product: ProductInfo?) {
        guard let product else { return }
        weeklyLabel.stringValue = "Lifetime"
        currentPriceLabel.stringValue = product.displayPrice
        ...
        basicPlaneLabel.stringValue = "\(Int(discount))%" + "Off"
