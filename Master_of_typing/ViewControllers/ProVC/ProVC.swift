...
class ProVC: BaseVC {
    ...
    @IBOutlet weak var startForFreeLabel: NSTextField!
    @IBOutlet weak var proBannerBox: NSBox!
    @IBOutlet weak var purchaseButtonBox: NSBox!
    ...
    override func viewDidLoad() {
        ...
        purchaseButtonBox.addZoomAnimation(duration: 0.8, scale: 0.97)

    }
    
    override func viewDidLayout() {
        collectionView.collectionViewLayout?.invalidateLayout()
        purchaseButtonBox.setAnchorPoint(anchorPoint: CGPoint(x: 0.5, y: 0.5))
        
    }
    
    override func viewDidDisappear() {
        purchaseButtonBox.removeZoomAnimation()
    }
    ...
    func updateStartForFreeLabel() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if selectedProduct.haveFreeTrial {
                ...
                threeDayFreeTrialLabel.stringValue = "\(selectedProduct.trialDays) " + "Days Free Trial, then".localized() + " \(selectedProduct.displayPrice) / " + "Monthly".localized()
                startForFreeLabel.stringValue = "Start 3 Days Free Trial".localized()
            } else {
                threeDayFreeTrialLabel.isHidden = true
                threeDayFreeTrialLabel.stringValue = ""
                startForFreeLabel.stringValue = "C O N T I N U E".localized()
            }
        }
    }
    ...
    @IBAction func startForFreeBtnAction(_ sender: Any?) {
        ...
        Task {
            do {
                ...
                showAlert(title: "Error".localized(), message: "Failed to purchase, Please try again.".localized())
            }
            hideHud()
        }
    }
...
extension ProVC: NSCollectionViewDataSource, NSCollectionViewDelegate, NSCollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: NSCollectionView, itemForRepresentedObjectAt indexPath: IndexPath) -> NSCollectionViewItem {
        ...
        if index == 0 {
            ...
            let string = "Save upto 50%, than".localized() + " \(cell.perWeekPriceLabel.stringValue)"
            threeDayFreeTrialLabel.stringValue = string

        } else if index == 1 {
            cell.configure(monthly: storeManager.monthly)
            let string = "\(cell.basicPlaneLabel.stringValue), " + "than".localized() + " \(cell.perWeekPriceLabel.stringValue)"
            threeDayFreeTrialLabel.stringValue = string
