...
class LessonVC: BaseVC {
    ...
    @IBOutlet weak var languageCountryFlag: NSImageView!
...
//            }
        ...
        languageLabel.stringValue = App.currentLanguage?.languageName ?? ""
        updateLanguageLabel()
        updateRowLabels()
    ...
    private func updateRowLabels() {
        ...
        lblHomeRow.stringValue = "Home Row".localized() + " (\(home.completed)/\(home.total))"
        
        let top = progress(for: "Top Row")
        lblTopRow.stringValue = "Top Row".localized() + " (\(top.completed)/\(top.total))"
        
        let bottom = progress(for: "Bottom Row")
        lblBottomRow.stringValue = "Bottom Row".localized() + " (\(bottom.completed)/\(bottom.total))"
        
        let different = progress(for: "Different Rows")
        lblDifferentRow.stringValue = "Different Rows".localized() + " (\(different.completed)/\(different.total))"
        
        let full = progress(for: "Full Keyboard")
        lblFullKeyBoardRow.stringValue = "Full Keyboard".localized() + " (\(full.completed)/\(full.total))"
    ...
    @IBAction func didTapLanguage(_ sender: NSButton) {
        createLanguageMenu(for: sender)
    }

    private func createLanguageMenu(for button: NSButton) {
        let menu = NSMenu()

        let systemItem = NSMenuItem(title: "🌍  System Default", action: #selector(selectLanguage(_:)), keyEquivalent: "")
        systemItem.target = self
        systemItem.state = (App.appLanguageCode == nil) ? .on : .off
        systemItem.representedObject = nil
        menu.addItem(systemItem)

        menu.addItem(NSMenuItem.separator())

        for language in languages {
            let title = "\(language.flagEmoji)  \(language.languageName)"
            let item = NSMenuItem(title: title,
                                 action: #selector(selectLanguage(_:)),
                                 keyEquivalent: "")
            item.target = self
            item.representedObject = language
            item.state = (language.code == App.appLanguageCode) ? .on : .off
            menu.addItem(item)
        }
    ...
    @objc private func selectLanguage(_ sender: NSMenuItem) {
        ...
        if sender.representedObject == nil {
            selectedCode = nil  // System default
        } else if let language = sender.representedObject as? Languages {
            selectedCode = language.code
        } else {
            return
        }
    
        App.appLanguageCode = selectedCode
        
        updateLanguageLabel()
    }
    
    private func updateLanguageLabel() {
        if let current = App.currentLanguage {
            languageCountryFlag.image = emojiToImage(current.flagEmoji, size: 24)
            languageLabel.stringValue = current.languageName
        } else {
            languageCountryFlag.image = NSImage.world
            languageLabel.stringValue =  "System Default"
        }
    }

    // Helper - convert emoji to NSImage (add this somewhere, e.g. in extension or utility file)
    func emojiToImage(_ emoji: String, size: CGFloat = 24) -> NSImage? {
    ...
    @IBAction func settingMenuBtn(_ sender: NSButton) {
        ...
        upgradeItem.title = "Upgrade to Pro".localized()
        ...
        restoreItem.title = "Restore Purchase".localized()
...
//        menu.addItem(NSMenuItem.separator())
        ...
        rateItem.title = "Rate us".localized()
        ...
        shareItem.title = "Share us".localized()
        ...
        supportItem.title = "Support".localized()
    ...
    @objc  func restorePurchasesTapped(){
        ...
        Task {
            do {
                ...
                hideHud()
                showAlert(title: "Error".localized(), message: "Failed to restore purchase, please try again later.".localized())
            }
        }
    }
...
extension LessonVC: NSCollectionViewDataSource, NSCollectionViewDelegate, NSCollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: NSCollectionView, itemForRepresentedObjectAt indexPath: IndexPath) -> NSCollectionViewItem {
        ...
        cell.lblTitle?.stringValue = "Lesson".localized() + " \(lesson.lessonNumber)"
